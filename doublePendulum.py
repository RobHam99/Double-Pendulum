...
def acc2(theta_1, theta_2, mass1, mass2, length1, length2, omega_1, omega_2, g):
    ...
    return float((t2_1 * (t2_2 + t2_3)) / t2_d)
...
class pendulum:
    ...
    def calculate(self):
        ...
        change_x = float(self.length_1 * np.sin(self.angle_1) + origin_x)
        change_y = float(self.length_1 * np.cos(self.angle_1) + origin_y)

        change_x2 = float(change_x + self.length_2 * np.sin(self.angle_2))
        change_y2 = float(change_y + self.length_2 * np.cos(self.angle_2))
